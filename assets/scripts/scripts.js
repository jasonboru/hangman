...
(function() {
   ...
   */
  $("#keypad").on("click", ".letter-button", pickLetter);
  ...
  function handlePickedLetter(letterPicked) {
    ...
    //if letterBlock is not greater than 0 put the letter in the graveyard
      ...
      document.getElementById("grave").appendChild(domElem);
  ...
  function displayGameOverMessageOnLose() {
    var incorrectlyGuessedLettersCount = $("#grave > div").length;
  ...
  function resetkeypad(){
    $("#keypad > .letter-disabled").each(function(index, element){
      $(element).removeClass().addClass('letter-button');
    });
  }

  function removeGraveyardLetters(){
    $('#grave > div').each(function(index, element){
      $(element).remove();
    });
  }
